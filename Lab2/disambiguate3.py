...
with open(outputFile, 'w', encoding="utf-8") as output:
    for page in Parser(wikipediaFile):
        ...
        page_content=page_content.lower().split(" ")
        s=set()
        for pc in page_content:
          if pc not in ['very', 'have', 'in', 'of', 'that' ,'from', 'what' ,'most' ,'which' ,'until' ,'only' ,'been' ,'their', 'only' ,'then']:
            s.add(pc)
        #print(s)
        #pc=page_content.replace("[^\\w\\s]"," ").split(" ")
        #content_set=set(pc)
        Entity = wikidata.rlabels[page_label]
        max=0
        score=0
        res='null'
        for e in Entity:
          k=0
          entity=wikidata.links[e]
          for ee in entity:
            ll=wikidata.labels[ee]
            for l in ll:
              sublabels=l.lower().split(" ")
             # label
              #for sub in sublabels:


              size=len(sublabels)
              sublabels=list(set(sublabels).intersection(s))
              if len(sublabels)>0 :
                k=k+len(sublabels)/size
                break
          score=k/len(entity)
          if score>max:
            max=score
            res=e
        print(page_title,res,sep='\t',file=output)
